...
String animal = "171024_mouseKdS1trainC";        
...
void drawAxes() {
  // DRAW and label graph axes
    ...
    textFont(font, 16);
    text(panel1title, 10, 20); // licks
    text(panel2title, 10, height/4+20); // laps
    text(panel3title, 10, height/2+20); // rews
...
void parseString() {
  ...
  String event6string = "numRew=";
  String event7string = "rewPos=";
  String event8string = "rewWidth=";
  ...
  if (event.contains(event1string)) {  // see what type of event
  ...
  else if(event.contains(event6string)) {  // numRew
    int evLen = event6string.length();
    int evInd = event.indexOf(event6string);
    String evTxt = event.substring(evInd+evLen, event.length()-2);
    numRew = int(evTxt);
    rewPos = expand(rewPos, numRew);  // have to expand initial size of reward array based upon numRew
  } // end ELSE for event type
  else if(event.contains(event7string)) {  // rewPosArr
    ...
    for (int i=0; i<numRew; i++) {
       //int evLen = event7string.length(); //println(evLen);
       evInd = event.indexOf(',', evInd+1); //println(evInd);
       if (i==0) {
         evTxt = event.substring(evLen+1, evInd); //println(evTxt);
         prevInd = evInd;
       }
       else { 
         evTxt = event.substring(prevInd+2, evInd); //println(evTxt);
         prevInd = evInd;
       }
       rewPos[i] = int(evTxt);
  ...
  else if(event.contains(event8string)) {  // rewWidth
...
void updatePosition() {
  ...
  for (int i=0; i<numRew; i++) {
    int rewXbeg = round(float(rewPos[i])/trackLen*(posPlotWidth)+posPlotXorig);
    int rewXend = round(float(rewPos[i]+rewWidth)/trackLen*(posPlotWidth)+posPlotXorig);
...
void updateNumbers() {
  ...
  noStroke();
  rect(80, 2, 60, 30);
  rect(80, height/4+2, 60, 30);
  rect(100, height/2+2, 60, 30);
  ...
  text(totalLicks, 110, 20); // licks
  text(totalLaps, 110, height/4+20); // laps
  text(totalRew, 130, height/2+20); // rews
